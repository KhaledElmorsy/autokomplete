...
function autocompleter(entries) {
  ...
  function build(entries) {
    ...
    function getCharacterArray(entries) {
      return entries
        .map(({ string }) =>
          [...util.normalizeString(string)] // Spread because '.split()' splits some emoji's in two.
    ...
    newEntries = newEntries.filter((entry) => {
      if (filters) {
        for (let filter of filters) {
          if (!filter(entry)) return false; // Only return if it fails a check, if it passes, check the other criterea.
        }
      }
      if (strings) {
  ...
  function match(query) {
    query = [...util.normalizeString(query)]; // Spread to maintain emojis, which have a length of two and mess with index matching
    ...
    function binarySearch(query, {start = 0, end = suffixArray.length - 1,lastMatch = false} = {}) {
      ...
      if (isMatched) {
        ...
        [start, end] =
          query.join('') < lcp + nextChar ? [start, midpoint - 1] : [midpoint + 1, end];
        return binarySearch(query, {start, end, lastMatch});
      }
    }
